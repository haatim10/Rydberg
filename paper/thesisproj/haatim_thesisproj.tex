...
\begin{abstract}
Rydberg atomic receivers measure the amplitude of an incident radio-frequency
field without directly observing its phase, making complex channel estimation a
challenging phase-retrieval problem. This paper investigates when the known
spatial structure of a multipath channel can improve such magnitude-only
channel estimation. We propose a Hankel-structured extension of
expectation--maximization Gerchberg--Saxton (EM-GS) that exploits the
relationship between channel coefficients across a uniform linear array. The
method selects the amount of structure using held-out pilot measurements and
falls back exactly to the original EM-GS estimator when the measurements do not
support a useful constraint. Our analysis and simulations show that structural
information is most beneficial when the channel is simple relative to the
structural capacity of the array, while its benefit decreases as channel
complexity approaches that capacity. We further show that effective channel
rank relative to array capacity describes this behaviour more consistently than
the number of propagation paths alone and remains predictive on previously
unseen channel distributions. These results provide a practical explanation of
when spatial structure should be exploited in magnitude-only channel estimation
and when an estimator should instead rely on the unconstrained solution.
\end{abstract}
...
\begin{figure*}[!t]
\centering
\resizebox{0.86\textwidth}{!}{\input{fig/fig0_system.tex}}
...
\end{equation}
which does not depend on $L_k$. This matters for the experiment in
Sec.~\ref{sec:pathcount}, where we change the number of paths. Without this
scaling, adding more paths would also add more received power, and we could not
tell whether a change in performance came from the extra complexity or simply
...
Therefore, when the channel has only a few important paths, its Hankel matrix
can have a low rank~\cite{cadzow1988,markovsky2008}. This is why we use the
Hankel representation: it changes the multipath structure of the channel into a
...
three important paths, its Hankel rank can be at most 3. This is much smaller
...
EM-GS estimates the channel from the magnitude measurements, but it does not
follow the rank structure discussed earlier. To include this structure, we
apply a Cadzow denoising step to the EM-GS channel estimate. There are three
main steps. First, the channel estimate is converted into the Hankel matrix
described earlier.
...
The estimate from EM-GS is then adjusted using this step. We call the new
approach Hankel-structured EM-GS (HS-GS). Choosing $r$ is important because a
rank that is too large may remove very little, while a rank that is too small
may remove useful parts of the channel.
...
selected rank is $\rmax$, all singular-value components are kept and the Cadzow
step does not lower the rank. In this case, HS-GS goes back to the original
EM-GS estimator.
...
The Hankel structure also allows us to use the larger array differently. The
channel coefficients across the array are not completely independent because
they are produced by the same propagation paths. The Hankel constraint uses
this relationship between the array elements. This allows HS-GS to make use of
the additional spatial structure provided by a larger array.
...
\begin{equation}
\mathrm{CCRB}=U(U^TJU)^{-1}U^T,
\end{equation}
where $J$ is the Fisher information matrix calculated in the previous section.
The matrix $U$ describes the ways in which the channel is allowed to change
while still following the multipath channel model.

We use both the CRLB and CCRB only as reference curves. The ordinary CRLB
treats the channel coefficients separately, while the CCRB also uses the known
multipath structure. The difference between them therefore gives us an idea of
how much improvement may be possible by using this structure.

% [NEW] What the CCRB curve does and does not specify. The review asked for a
% full derivation; this states honestly what is and is not given here.
\textbf{What we state here is not enough to reproduce these curves.} A reader
who wants to redraw them would also need the real parameter vector we
differentiate with respect to, the exact likelihood, the derivative
expressions, the dimensions of $U$ and the basis chosen for it, the rank
conditions under which $(U^TJU)$ is invertible, and the normalisation used to
turn the bound into an NMSE. We do not give those details, so the curves should
be read as indicative references only and not as verified bounds. The
distinction also matters because a bound for the physical multipath model of
\eqref{eq:chan} is not the same as a bound for a fixed-rank Hankel model, and
the two should not be used interchangeably.

These bounds are derived for unbiased estimators, while EM-GS and HS-GS are not
assumed to be unbiased. Therefore, we do not treat the CRLB or CCRB as absolute
lower limits on their measured NMSE.
...
For each trial, we see how much better HS-GS performs compared with EM-GS. We
then use the median improvement across all trials as the main performance
measure. Most experiments are performed at fixed SNR values. For the
effective-rank experiments, the SNR is randomly selected between $-10$ and
20~dB for each trial, and the results are then grouped according to SNR.

Table~\ref{tab:config} summarizes the four simulation configurations used in
the experiments.

\begin{table}[!t]
\caption{Simulation configurations used in the numerical experiments.}
\label{tab:config}
\centering
\footnotesize
...
\midrule
A & 100 & 25 & 10, 30 & 12 & TB & $\{-5,\ldots,20\}$ \\
B & 50 & 20 & 30 & 12 & TB & $\{-5,\ldots,20\}$ \\
...
that HS-GS has a larger total squared error than EM-GS.
...
improves over EM-GS by 7.04~dB. As the number of paths increases, the
improvement becomes smaller. At $L=16$, the gain is $-0.12$~dB, which means
that the Hankel constraint no longer helps.
...
\end{equation}
for $N=16$, 32, and 64, respectively. Although the array size increases from 16
to 64, these values stay relatively close to each other. In the cases tested
here, this suggests that the Hankel constraint is most useful when the channel
effectively uses about half or less of the available Hankel rank.
...
$\rho=\reff/\rmax$ for $N\in\{16,32,64\}$. The gain approaches zero at similar
values of $\rho$ for the three array sizes.}
\label{fig:rho}
\end{figure}
...
Cadzow step was forced to remain active. At $\rho=0.735$, the gain becomes
$-0.075$~dB. The negative gain shows that forcing a low-rank constraint on a
complex channel can make the estimate worse. The adaptive rank selection helps
prevent this problem because HS-GS can fall back to EM-GS.

These results suggest that $\rho$ is a useful way to describe when the Hankel
constraint helps. However, it does not completely determine the amount of
improvement, since the exact HS-GS gain still changes with array size. Also,
$\rho$ is calculated here using the true noiseless channel, which is not known
at the receiver. We therefore use $\rho$ to understand the simulation results
rather than as something that the estimator uses directly.
...
Finally, we test whether the Cadzow step needs to be applied repeatedly during
the EM-GS iterations. We compare two approaches. In the first approach, the
step is applied during the EM-GS iterations. In the second approach, EM-GS is
completed first and the Cadzow step is applied only once at the end.

The difference in mean gain between these two approaches is only 0.003~dB. This
is a very small difference. It suggests that, in the cases tested here, the
biggest benefit comes from applying the Hankel structure itself rather than
from applying the step repeatedly during the EM-GS iterations. Since the
one-shot version is also cheaper, this is a useful practical result, although a
small difference in the mean does not prove that the two are equivalent at
...
We therefore use the normalized effective rank
\begin{equation}
\rho=\frac{\reff}{\rmax},
\end{equation}
which compares the effective rank of the channel with the maximum rank of the
Hankel matrix. For example, $\rho=0.25$ means that the channel effectively uses
only about one quarter of the available rank.

For $N=16$, 32, and 64, the HS-GS gain approaches zero at $\rho=0.588$, 0.518,
and 0.544, respectively. These values stay roughly the same even when the array
size increases by a factor of four. In the cases tested here, this suggests
that the Hankel constraint is most useful when the channel effectively uses
about half or less of the available Hankel rank.

The relationship is also examined using two additional channel distributions.
The predicted HS-GS gains differ from the measured gains by 0.13 and 0.315~dB.
These findings give more evidence that normalized effective rank is a useful
way to understand when the Hankel structure helps within the channel model
considered here.
...
Section~III-B. It is also a weaker requirement than the array model itself, so
