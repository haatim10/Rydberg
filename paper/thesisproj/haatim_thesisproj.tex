...
\begin{tabular}{crcc}
\toprule
SNR (dB) & Inter. $-$ post-hoc (dB) & 95\% CI & Median $q$ \\
\midrule
...
$N=32$, the HS-GS gain falls from $+7.04$~dB when $L=2$ to $+0.046$~dB when
$L=14$, where its interval first contains zero, and to $-0.117$~dB with an
interval excluding zero when $L=16$, where the constraint is doing measurable
harm rather than simply failing to help. However, the number of paths alone is not a complete measure of channel
