...
distinguish from signal~\cite{cadzow1988,markovsky2008}. This works, and it is
...
\textbf{Relation to rank projection in projected gradient descent.} Projecting
onto a low-rank set inside an iterative estimator is not new. Xu \emph{et
al.}~\cite{xu2025} place a rank projection inside projected gradient descent
(PGD) for this setting and report that ``when the pilot length is small
($P=10$), the PGD performs better than the basic GD, as the projection
operation in PGD helps to exploit the rank constraint.'' Our pilot sweep
(Sec.~\ref{sec:results}) finds the same qualitative trend for a different
constraint, which we take as corroboration rather than contrast.

The distinction that matters is \emph{which} matrix is constrained. Their rank
constraint applies to the channel matrix of a two-dimensional ($8\times8$)
array, limiting the number of distinct spatial signatures; ours applies per
user, to the Hankel embedding of a single column, and encodes the harmonic
structure of the ULA manifold, so it is informative even at $K=1$. The two are
complementary rather than substitutes. They are also not available at the same
scale: their one-dimensional array has $I=8$ elements, for which
$\mathrm{cap}(8)=4$ by \eqref{eq:cap}, so the embedding admits at most four
ranks and the regime this paper characterises barely exists there. The
structure we exploit needs aperture before it has anything to say.
...
Users $K$ & 3~\cite{xu2025} (2, 4 in the $K$ sweep) \\
...
receive-side Saleh--Valenzuela configuration of~\cite{precoding2408} ---
$L=10$ paths, $\alpha_\ell\sim\CN(0,1)$, incident angles
$\mathcal{U}(-90^\circ,90^\circ)$.~\todo{FILL IN AUTHORS of arXiv:2408.14366
from page 1 of the PDF; it is not readable from the environment these results
were produced in} That work is a \emph{precoding} study, not a competing
estimator, and we borrow only its channel configuration. Two properties make
it a good test: it was specified by other authors for a different purpose, and
it uses uniform linear arrays at both ends with both incident and departure
angles, of which we use the receive side only. At
