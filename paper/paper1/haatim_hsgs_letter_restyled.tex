...
\begin{abstract}
Multi-user channel estimation is studied for an atomic receiver that measures
only the magnitude of the incident field, in the presence of a known reference
beam. The problem is one of biased phase retrieval. Its Fisher information is
shown to be rank one per measurement and block diagonal across the receive
elements, so the normalised per-element information does not grow with the
array size. Aperture is therefore usable only through cross-element structure.
On a uniform linear array the Hankel lifting of each channel column supplies
such a structure, with a capacity of $\lceil N/2\rceil$ ranks. A rank prior is
accordingly predicted to help only below a small fraction of that capacity, and
to harm above it. An estimator enforcing the structure inside an exact
expectation--maximization Gerchberg--Saxton iteration is proposed, the model
order being selected from a held-out pilot residual. The predicted behaviour is
observed. In summed squared error, averaged over twelve operating points per
aperture, the gain is $+2.93$~dB at $N=32$ and $-0.25$~dB at $N=8$, while the
unstructured baseline varies by at most $0.163$~dB. The reversal belongs to
that statistic alone: under a paired per-trial median the same $N=8$ cells give
$+0.225$~dB, the selector abstaining in $42\%$ of trials. The governing
variable is shown to be the effective rank relative to capacity, not the path
count. Used on two channel distributions to which it was not fitted, it
predicts the gain to within $0.13$ and $0.315$~dB. Results are simulation only;
the derived bounds do not govern biased estimators.
\end{abstract}
