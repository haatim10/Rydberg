...
solvers are flat in array size: under the unconstrained row-wise
parameterisation the Fisher information is block diagonal across receive
elements, so normalised per-element information does not grow with $N$ and
exploiting aperture requires cross-element structure. We
...
$3\sum_k L_k$-parameter manifold. The gain grows from $-0.19$ to $+2.85$~dB as
$N$ goes $8\to32$ while the unstructured baseline moves by $0.012$~dB. We then
show the governing variable is not path count but effective rank relative to
capacity, and use that relation to predict the gain on two channel-generation
distributions it was not fitted to. Simulation only; the bounds do not govern
biased estimators.
\end{abstract}
...
grows.} Under the unconstrained row-wise parameterisation of \eqref{eq:obs},
...
problem is unchanged by adding elements. Exploiting aperture therefore requires
\emph{cross-element} channel structure; HS-GS uses the low-rank Hankel
structure induced by the geometric ULA model, which is one such structure
...
is rank deficient. EM-GS numerically tracks the unconstrained bound to within
$0.051$~dB for SNR $\ge 5$~dB, so the benchmark is informative.
...
The relation is fitted on one channel-generation distribution and tested,
without refitting, on two previously unseen ones --- the clustered-propagation
falsification test this work's earlier stage identified as decisive.
\end{enumerate}
...
\label{eq:obs}
\end{equation}
elementwise, and the modulus is never linearised. Here ``biased phase
retrieval'' refers to the known reference-field offset $\bB$ and is distinct
from the statistical estimator bias discussed in \S\ref{sec:crlb}. User $k$'s
channel is
\begin{equation}
...
total at $K=3$; $10$~dB single-user is $5.23$~dB. The empirically measured
conversion is slightly larger than the nominal $4.77$~dB --- $4.85$~dB over
$300$ realisations --- which places the $12$~dB case at $7.15$~dB when measured
rather than computed; the difference is finite-sample and we quote the nominal
value throughout.
% src: trackD_urformer/config.py:143-145, measured RSR_ours 10.06 dB,
% RSR_paper 5.21 dB over 300 realizations
All RSR values quoted here are single-user.
...
effective rank~\cite{royvetterli2007} of the noiseless
...
cross-generator predictions of \S\ref{sec:oom} possible. It must be computed on
...
$N$ & 8, 16, 32 & 16, 32, 64 \\
$K$ & 3 & 3 \\
$P$ & 30 (Figs.~1,~3); 10 and 30 (Fig.~2) & 20 \\
$L_k$ & $\mathcal{U}\{3,7\}$~\cite{xu2025} & $\mathcal{U}\{3,7\}$~\cite{xu2025} \\
SNR & $\{-5,0,5,10,15,20\}$~dB & $\mathcal{U}[-10,20]$~dB \\
RSR (single-user) & 12~dB & 10~dB \\
Iterations $T$ & 50 & 100 \\
Cadzow sweeps & 4 & 1 \\
Statistic & ratio-of-sums & paired median, CI \\
Trials & 400 per point, $1.44\times10^4$ total & 83--874 per cell \\
\bottomrule
\end{tabular}
\end{table}
...
against; the effective-rank and cross-generator results are reported at their own
configuration, and each figure names the one it uses.

\subsection{Main result against the bounds}

At $N=32$, $P=30$, RSR $12$~dB (Fig.~\ref{fig:snr}) EM-GS numerically tracks
the unconstrained CRLB to within
% src: results/track_b/b3/N32_P30_snr*.npz (400 trials/point, ratio-of-sums)
% differenced against results/track_b/constrained_crlb.json, same trials
$0.051$~dB for SNR $\ge5$~dB. We do not call it efficient: it is biased, and
the bound formally governs unbiased estimators (\S\ref{sec:crlb}); the close
numerical agreement is what makes the benchmark informative. At $-5$ and
$0$~dB the curves separate by $0.295$ and $0.237$~dB, consistent with the
low-SNR bias of a shrinking update. At this configuration the geometric CCRB
lies $7.05$--$7.11$~dB below the unconstrained bound, quantifying what the
structure is worth in principle, and HS-GS occupies part of that gap,
...
rank-one unconstrained CRLB to within $0.051$~dB above $5$~dB. Neither bound
...
operating points at each array size --- the six SNR values of
Table~\ref{tab:params} at each of $P\in\{10,30\}$ --- the gain is
% src: results/track_b/b3/N{8,16,32}_P{10,30}_snr*.npz, 400 trials/point;
% per-operating-point mean, the same pooling scripts/plot_paper.py::fig2 draws
$-0.19$, $+0.78$, $+2.85$~dB at $N=8,16,32$, with win rates $33.5$, $74.0$,
$95.3\%$ and the constraint active in $57.5$, $92.7$, $99.6\%$ of trials.
\textbf{Under the same averaging EM-GS varies by $0.012$~dB}, exactly as the
...
$L_k\in\{3,\dots,7\}$, so the projection is inactive in $42.5\%$ of trials and
where active often truncates real components;
% src: results/track_b/b3/N8_P{10,30}_snr*.npz
HS-GS gains $+0.78$ and $+1.47$~dB at $-5$~dB (at $P=30$ and $P=10$) but turns
negative as SNR rises, reaching $-2.23$~dB, so the sign of any scalar summary
depends on the pooling. Truncation bias does not shrink with
...
configuration. Two pilot counts are shown: $P=30$, which Figs.~1 and~3 also
use, and $P=10$ as an auxiliary curve. Faint markers are the six individual
SNR operating points of each curve; solid lines are their means. The headline
$-0.19/+0.78/+2.85$~dB is the mean over all twelve points (six SNR values
$\times$ two pilot counts). At $N=8$ the points straddle zero, so no single
scalar summarises that array size.}
\label{fig:scaling}
\end{figure}
...
\subsection{Cross-generator prediction: the falsification test}
\label{sec:oom}
...
\textbf{The prediction rule, stated so it can be reproduced.} The relation is
the eight-point table of $(\reff/\rmax,\ \dhs)$ pairs obtained by re-indexing
the path-count sweep of \S\ref{sec:results}-C, and the predicted $\dhs$ is its
\emph{piecewise-linear interpolation} at the measured $\reff/\rmax$ --- no
curve is fitted, no regression or spline is involved. The prediction interval
is that same interpolation evaluated at the interquartile endpoints of
$\reff$ over the channel realisations, so it reflects channel variability
rather than being asserted. Both the table and the rule were fixed before
either measurement below.
...
paths, $\alpha_\ell\sim\CN(0,1)$, angles $\mathcal{U}(-90^\circ,90^\circ)$. That work is a
...
effective-rank configuration. Both generators produce channels of the
form \eqref{eq:chan}; what differs is the distribution over
$(L_k,\alpha_{k\ell},\theta_{k\ell})$. The relation was fitted on the
generator of \S\ref{sec:results} alone and not refitted afterwards.}
\label{fig:oom}
\end{figure}
...
measures $+2.85$~dB under the bound-comparison configuration and $+2.680$~dB
under the effective-rank one --- different code, different RSR, iteration
count, Cadzow sweep count and pooling rule, agreeing to $0.17$~dB.
...
elements under the unconstrained row-wise parameterisation, the unstructured
baselines are flat in array size while HS-GS converts a larger aperture into
lower normalised error: $-0.19\to+2.85$~dB as
$N$ goes $8\to32$ against $0.012$~dB of baseline movement. The governing
variable is effective rank relative to capacity, not path count, and that
relation predicted the gain on two unseen channel-generation distributions ---
