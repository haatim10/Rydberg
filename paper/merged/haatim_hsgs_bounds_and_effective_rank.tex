...
unconstrained CRLB to within
% src: paired per-trial store trackB_hankel_emgs/results/grid/N32_P30_snr*.npz
% (200 trials/point, ratio-of-sums) differenced against
% results/track_b/constrained_crlb.json on the same trial indices
$0.164$~dB for SNR $\ge 5$~dB, so it is close to efficient for the unstructured
problem and the benchmark is credible; at $-5$ and $0$~dB the curves separate
by $0.442$ and $0.239$~dB, consistent with the low-SNR bias of a shrinking
update. At this configuration the geometric CCRB lies $7.05$--$7.11$~dB below
the unconstrained bound, quantifying what the structure is worth in principle,
and HS-GS occupies part of that gap, improving on EM-GS by
$2.24$--$3.09$~dB across the sweep.
...
rank-one unconstrained CRLB to within $0.164$~dB above $5$~dB. Neither bound
