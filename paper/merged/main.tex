...
efficient for the unstructured problem and the benchmark is credible; at $-5$
and $0$~dB the curves separate by $0.30$ and $0.24$~dB, consistent with
low-SNR bias. The geometric CCRB lies $0.87$--$9.98$~dB below the
unconstrained bound, quantifying what the structure is worth in principle, and
HS-GS occupies part of that gap, improving on EM-GS by $2.27$--$3.04$~dB.
...
rank-one unconstrained CRLB to within $0.05$~dB above $5$~dB. Neither bound
...
Fig.~\ref{fig:scaling} isolates the scaling. Averaged over twelve operating
points per array size the gain is
% src: trackB_hankel_emgs/results/experiment_B_array_size.csv
...
$L_k\in\{3,\dots,7\}$, so the projection is inactive in $43\%$ of trials and
where active often truncates real components; HS-GS gains $+0.744$~dB at
...
converts a larger aperture into lower normalised error: $-0.19\to+2.85$~dB as
