...
% \stub{} blocks, each naming what it waits on. They wait on batch C2, because
% the three possible outcomes are three different papers and those three
% sections are where a paper commits to one.
%
...
%
%   +1.209 dB  remains a valid measurement OF ONE SEED. Whether it is typical
%              of its own seed distribution is unmeasured; batch C2 measures it.
%
...
% TITLE IS A STUB (PROMPT 19 E2). The old title names structural priors as the
% subject, and whether they are the subject depends on which P29 outcome fires:
%   (a) mixed Delta_H stable near +1.2  -> the paper is about a prior whose
%       value depends on training adequacy, and the old title stands;
%   (b) mixed Delta_H also sign-unstable -> the paper is about a quantity the
%       literature reports at a resolution it does not have, and the title must
%       say so;
%   (c) mixed Delta_H stable but smaller -> the paper is a loss-design paper
%       with a corrected structural-prior figure, and the title should lead
%       with the loss.
% Placeholder below is the working title, not a decision.
...
\begin{abstract}
\textcolor{red}{[ABSTRACT IS A STUB --- PROMPT 19 E2.]} It cannot be written
until batch C2 settles which of the three P29 outcomes fires, because the
three are three different papers and the abstract is the sentence that commits
to one. What it must contain either way: the gradient-concentration finding
($89.7\%$ of the gradient below $5$~dB, a per-bin share spanning a factor of
$30.0$); the matched-adequacy protocol; the corrected loss and its
$+2.171$~dB three-seed gain at SNR~$\ge5$~dB with no low-SNR cost; the
pilot-efficiency distinction at $2.231$~dB; and simulation-only scope. What it
must not contain until C2 reports: any scalar for the structural gain.
\end{abstract}
...
SNR concentrates the gradient at low SNR, so any mechanism that improves
high-SNR behaviour will appear to add information. We give a matched-adequacy
protocol that separates the two.
\item A corrected loss that is not a workaround but a fix, gaining
$+2.171$~dB at SNR~$\ge5$~dB over three seeds with no low-SNR penalty ---
indeed with a gain in every bin of every seed, including the two bins the
weighting penalizes most.
...
\item \textcolor{red}{[STUB]} The structural-prior contribution, pending C2.
\end{enumerate}

\stub{INTRODUCTION, structural-prior claim}{One paragraph stating what the
control did to $\Delta_H$, and the corresponding contribution item. It cannot
be written until batch C2 reports, because the three P29 outcomes give three
different claims: a stable gain whose value depends on training adequacy; a
quantity with no stable sign under any design; or a stable gain materially
smaller than first reported. What is already established and belongs here
whichever fires: under focused training $\Delta_H$ has no stable sign across
three seeds, and the per-bin ordering --- a cost at low SNR, a benefit at high
SNR --- replicates on every arm measured while the SNR at which it crosses does
not.}
...
did to $\Delta_H$, and its figure. Waiting on batch C2, which measures the
seed spread of the mixed-SNR contrast; the focused contrast is already measured
on three seeds. No scalar may be quoted here until then, and the figure must be
rebuilt from C2 and C3 rather than repurposed --- the retired one plotted a
single seed of each design as though the pair were a measurement.}
...
\stub{STRUCTURAL-PRIOR SECTION}{The whole subsection, pending batches C2 and
C3. Three things are already established and will survive whichever P29
outcome fires, and they are recorded here so the section is written from them
rather than around them. First, under focused training $\Delta_H$ has no
stable sign: three seeds give $+0.078$, $+0.057$ and $-0.076$~dB, every
bootstrap CI over test realizations excluding zero, and the across-seed SD is
$0.083$~dB --- small, so this is a statement about the effect and not about
noise. Second, the paired per-trial median and the ratio of sums disagree in
sign, and the reason is measured: the sum of squared errors is dominated by
low-SNR realizations, which is exactly where the prior costs accuracy. It is
\emph{not} the abstention mechanism of the classical estimator --- the
projection is active on every layer of every one of $2000$ trials on every
seed. Third, the per-bin ordering, a cost at low SNR turning into a benefit at
high SNR, replicates on every arm measured, across two initializers, two
training designs, two losses and four seeds; the SNR at which it crosses does
not, moving across three bin boundaries. The reportable object is that
ordering, not any scalar.}
...
Per-sample normalized error over a mixed-SNR training set concentrates
$89.7\%$ of the gradient below $5$~dB and leaves the high-SNR regime
underfitted, so any mechanism that improves high-SNR behaviour --- a structural
prior among them --- will appear to add information it need not have supplied.
Rebalancing the loss, a change of a few lines with no architectural addition,
gains $+2.171$~dB at SNR~$\ge5$~dB across three seeds and improves every one of
eighteen bin-by-seed cells, including the two bins the weighting itself
penalizes most. We recommend the matched-adequacy control be run before a
structural prior is credited, and note that it cost us our own result first.

\stub{CONCLUSION, structural-prior sentence}{One or two sentences stating what
the control did to $\Delta_H$, pending C2. The claim the retired draft made
