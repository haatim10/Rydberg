...
remedy. They also observe, in their introduction, that systems are usually
trained either at a single SNR or on samples drawn uniformly across the target
range, apparently without the effect on performance at different SNRs being
questioned --- a published statement that the convention is unexamined, which
is the gap this paper works in. Work on deep OFDM channel estimation has
separately examined training on SNR-restricted samples, reporting the expected
trade of low-noise accuracy against high-noise
accuracy~\cite{jameel2024ofdm}. What we take from that line of work is the mechanism and the
...
The differences from~\cite{wiesmayr2023bler} are worth stating exactly. Their
weights are recomputed each epoch from inverse accumulated losses, with a
stability constant and a normalization at the centre of their SNR grid; ours
are computed once and never change. They also note at the end of their
Section~4 that one could instead deweight using the loss of a fixed baseline
such as a classical system, rather than the adaptive scheme they implement ---
a fixed-weight variant they raise but do not implement. Ours is fixed in that
sense, with the further difference that the weights come from the network's own
pre-training error and not from a separate classical baseline.
...
deweighting penalizes; where they examine it they report the trade. In their
narrow-range experiment, deweighted training holds a small advantage at high
SNR while naive training keeps a smaller one at the low end of the waterfall,
and their replication on a simpler system repeats the pattern. Their
wide-range result is a $0.54$~dB gain at $1\%$ block error rate, which is a
high-SNR operating point; no per-bin low-SNR improvement is claimed.

Three differences must be kept in view before that contrast is read as a
disagreement. \emph{Metric}: they measure bit and block error rate on a coded
MIMO-OFDM system, we measure normalized channel-estimation error, so
``improvement in the down-weighted bins'' is not the same quantity. \emph{
Mechanism}: their weights are recomputed each epoch from inverse accumulated
losses with a stability constant and a normalization at the grid centre; ours
come from one pass before training. \emph{Size}: their gain is $0.54$~dB, which
they describe as modest but essentially free since it comes from the loss
rather than a larger receiver or more data. Because the metrics differ, their
number and ours are not directly comparable and we do not compare them.
...
\begin{tabular}{lrrrr}
\toprule
SNR bin (dB) & mixed & SD over seeds & focused & balanced \\
\midrule
% src: reports/p19/p29_score.json three_seed_mean_per_bin_db
% src: balanced column reports/p20/p30_score.json, three-seed mean of
% per_seed.seed{1,2,3}.per_bin_db; cross-checked against
% reports/p20/PART_B_C3.md table B3.4.
$[-10,-5)$ & $-0.120$ & $0.036$ & --- & $-0.233$ \\
$[-5,0)$   & $-0.400$ & $0.103$ & --- & $-0.703$ \\
$[0,5)$    & $-0.293$ & $0.157$ & --- & $-0.944$ \\
% src: reports/p17/p28_score.json three_seed_mean_per_bin_db
$[5,10)$   & $+0.457$ & $0.052$ & $-0.173$ & $-0.692$ \\
$[10,15)$  & $+1.403$ & $0.089$ & $+0.067$ & $-0.241$ \\
$[15,20)$  & $+2.290$ & $0.082$ & $+0.259$ & $+0.567$ \\
\bottomrule
\end{tabular}
\end{table}
...
It is not, however, better balanced, and here a registered prediction of ours
was wrong in the direction of the error: we predicted the log loss would
over-correct \emph{less} than per-bin reweighting, and it over-corrects more.
Its realized share of gradient below
$5$~dB is
...
hardware and no recorded channel data. They also cover one architecture, so we
cannot separate what is a property of training design in general from what is a
property of this network; \cite{wiesmayr2023bler} repeats its whole experiment
on a second, simpler system precisely to rule that out, and the corresponding
check here has not been run. They cover the geometric ULA model of
